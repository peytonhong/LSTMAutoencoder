...
for i in range(-50,50):
    ...
    for j in range(seq_length):
        _input.append([i+j])
        _predinput.append([i+j+seq_length])
        #_input = [[i],[i+1],[i+2]]
        #predinput = [[i+3],[i+4],[i+5]]
    inputs.append(_input)
    predinputs.append(_predinput)
inputs = np.array(inputs, dtype=np.float32,)
predinputs = np.array(predinputs, dtype=np.float32,)

testX=np.array([[[5.5],[6.5],[7.5]]],dtype=np.float32)
predtestX=np.array([[[8.5],[9.5],[10.5]]],dtype=np.float32)
...
with tf.variable_scope("encoder"):
    ...
    _encoder_state = multi_encoder_cells.zero_state(1, dtype=tf.float32)
